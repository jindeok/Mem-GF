...
class PGSP:
    def __init__(
        ...
        # ========= [전처리: R_norm <- R_norm @ D^beta] =========
        # (예) 아이템 차원의 deg를 구해 diag(deg^beta)를 만들어 오른쪽 곱
        # 실제 논문에 따라 user/item 양쪽을 처리할 수도 있음
        if R_norm.is_sparse:
            R_norm_dense = R_norm.to_dense()
        else:
            R_norm_dense = R_norm
        
        # 아이템 차원 deg: 각 column 합
        ...
        # (참고) R_norm_modified는 filter 계산에만 사용했으니, 원래 R_norm도 보관 가능
        # 후처리에 필요하다면 저장
        self.item_deg_beta = item_deg_beta  # (n_items,) - 후처리용

        print("Filter computation done .. \n")


    def predict_batch(self, p_u: torch.Tensor, r_u: torch.Tensor) -> torch.Tensor:
        # ========== Preprocessing (논문 Eq.8) ==========
        # R_personalized = concat(P_user, R)
        # 여기서는 함수로 p_u, r_u가 이미 들어온다고 가정
        ...
        # ========== Postprocessing (논문 Eq.10) ==========
        # R_pred = R_filtered[:, n_users:]
        r_pred = out_mixed[:, self.n_users:]  # (batch_size, n_items)

        # 추가로, "R_norm <- R_norm @ D^-beta" 식에 해당하는 역연산을 배치 예측 결과에 적용
        # 예: r_pred = r_pred @ diag(deg^-beta)
